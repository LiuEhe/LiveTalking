...
## 5. 统一响应与异常接管 (Unified Response & Exception Handling)
- **拒绝裸露的 try...except**：在 API 层的所有端点函数中，严禁充斥冗杂的 `try...except...` 来捕获常规业务错误。业务代码只需专注于成功的逻辑，直接返回正确的响应内容即可。
- **使用自定义 APIRoute 处理**：统一使用类似 `Success` 类的高级请求封装（继承自 FastAPI 原生的 APIRoute 重写）。在定义路由对象时必须将其挂载（如 `APIRouter(route_class=Success)`），这样才能保障全局在底层毫无副作用地拦截崩溃、打印完整日志、并对前端输出无异构的格式规范（如 `{"code": -1, "msg": "..."}`）。
